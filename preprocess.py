...
parser.add_argument("--debug", default='false', type=str, required=True, help="Debug mode (True/False).")

args = parser.parse_args()

args.debug = False if args.debug.lower() == 'false' else True

logger.info(args)

for subdir in 'discharge 2days 3days'.split():
    os.makedirs(f'{args.output_dir}/{subdir}', exist_ok=True)

logger.info('loading admissions')

if args.debug:
    df_adm = pd.read_csv('s3://mimic-iii-physionet/ADMISSIONS.csv.gz', nrows=20000)
else:
    df_adm = pd.read_csv('s3://mimic-iii-physionet/ADMISSIONS.csv.gz')
...
if args.debug:
    df_notes = pd.read_csv('s3://mimic-iii-physionet/NOTEEVENTS.csv.gz', nrows=1000)
else:
    df_notes = pd.read_csv('s3://mimic-iii-physionet/NOTEEVENTS.csv.gz')
    
df_notes = df_notes.sort_values(by=['SUBJECT_ID','HADM_ID','CHARTDATE'])

logger.info('merging admissions and notes')
df_adm_notes = pd.merge(
        df_adm[['SUBJECT_ID','HADM_ID','ADMITTIME','DISCHTIME','DAYS_NEXT_ADMIT',
                'NEXT_ADMITTIME','ADMISSION_TYPE','DEATHTIME','OUTPUT_LABEL','DURATION']],
        df_notes[['SUBJECT_ID','HADM_ID','CHARTDATE','TEXT','CATEGORY']], 
        on = ['SUBJECT_ID','HADM_ID'],
        how = 'left')

logger.info(f'merged dataset: {df_adm_notes.shape}')


df_adm_notes['ADMITTIME_C'] = pd.to_datetime(df_adm_notes.ADMITTIME.apply(lambda x: str(x).split(' ')[0]), format = '%Y-%m-%d', errors = 'coerce')
...
def preprocessing(df_less_n): 
    ...
    for i in tqdm(range(df_len)):
        ...
        n = int(len(x)/318)
        for j in range(n):
            want = want.append({'TEXT':' '.join(x[j*318:(j+1)*318]), 
                                'Label':df_less_n.OUTPUT_LABEL.iloc[i], 'ID':df_less_n.HADM_ID.iloc[i]}, ignore_index=True)
        if len(x)%318 > 10:
            want = want.append({'TEXT':' '.join(x[-(len(x)%318):]), 
                                'Label':df_less_n.OUTPUT_LABEL.iloc[i], 'ID':df_less_n.HADM_ID.iloc[i]}, ignore_index=True)
    return want
...
assert len(pd.Index(id_test_t).intersection(pd.Index(id_train_t))) == 0
...
assert len(pd.Index(df).intersection(pd.Index(not_readmit_ID_use))) == 0

# for this set of split with random_state=1, we find we need 400 more negative training samples
not_readmit_ID_more = df.sample(n=1 if args.debug else 400, random_state=1)
...
#check if balanced
print(discharge_train_snippets.Label.value_counts())


discharge_train_snippets.to_csv(f'{args.output_dir}/discharge/train.csv')
discharge_val.to_csv(f'{args.output_dir}/discharge/val.csv')
discharge_test.to_csv(f'{args.output_dir}/discharge/test.csv')
...
early_train_snippets.to_csv(f'{args.output_dir}/3days/train.csv')
early_val = df_less_3[df_less_3.ID.isin(val_id_label.id)]
early_val.to_csv(f'{args.output_dir}/3days/val.csv')
...
early_test.to_csv(f'{args.output_dir}/3days/test.csv')
...
early_test_2days.to_csv(f'{args.output_dir}/2days/test.csv')